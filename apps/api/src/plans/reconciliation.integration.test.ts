...
describe('Nurture and source-version Plan reconciliation', () => {
  ...
  beforeAll(async () => {
    ...
    completedItemId = (await getPlanBuilder(prisma, clientId)).plan!.versions[0]!.items.find(
      ({ stableKey }) => stableKey === 'completed-guidance',
    )!.id;
  ...
  test('ignores non-material change and creates an explicit preserving replacement for material change', async () => {
    ...
    await expect(
      reconcilePlanSources(prisma, {
        ...
        sourceProfileVersion: 1,
        material: true,
        reason: 'Same source is still current',
      }),
    ).resolves.toMatchObject({ changed: false });
    const changed = await reconcilePlanSources(prisma, {
      clientId,
      planId,
      actorId: consultantId,
      sourceProfileVersion: 2,
      material: true,
      reason: 'Published profile materially changed',
    });
    ...
    expect(versions[0]).toMatchObject({
      status: 'STALE',
      staleReason: 'Published profile materially changed',
    });
    expect(versions[1]).toMatchObject({ status: 'DRAFT', supersedesVersionId: versions[0]!.id });
    expect(
      versions[1]!.items.find(({ stableKey }) => stableKey === 'completed-guidance'),
    ).toMatchObject({
      clientTitle: 'Protected approved guidance',
      consultantRationale: 'Manual rationale',
      manuallyProtected: true,
    ...
    expect(
      await prisma.workItem.count({
        where: { sourceType: 'PlanVersion', sourceId: changed.versionId },
      }),
    ).toBe(1);
  });
  ...
  test('replacement becomes active only through governed approval and supersedes old history', async () => {
    ...
    expect(
      await prisma.planItem.findFirst({
        where: { planVersionId: versions[1]!.id, stableKey: 'completed-guidance' },
      }),
