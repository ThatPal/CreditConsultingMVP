...
U1-27: affordances.ts projects availableActions (typed navigation versus Plan commands) and scope-specific Plan blockers from the same client-safe Plan basis. Command entries always require revalidation; no read grants permission. Locked-step ownership is null because prerequisite owners must not be guessed. Completed/cancelled items produce no blocker. This covers the Plan response subset only; Review/service/access and full source dependency contracts remain open. U4 retires this adapter. Evidence: affordances unit tests, Plan execution rejection after repeated workspace reads, Center/workspace agreement.
