...
export type SyntheticReport = {
  ...
  inquiries: Array<{ bureau: SyntheticTradeline['bureau']; creditor: string; date: string; page: number }>;
  negativeItems: Array<{ bureau: SyntheticTradeline['bureau']; kind: string; date: string; page: number }>;
};
...
const evidence = (bureau: string, page: number, label: string): Evidence => ({
  kind: 'direct_source', source: bureau, page, label,
});
const exception = (key: string, summary: string, evidenceItems: Evidence[] = []): AIException => ({
  key, category: 'REPORT_EXTRACTION', summary, materiality: 'material', confidence: 'low',
  evidence: evidenceItems, suggestedResolution: 'Consultant review required',
  humanReviewRequired: true, blockingBehavior: 'review',
});
...
export function validateSupportedReport(source: ReportSource, report: SyntheticReport) {
  ...
  return {
    ...
    detectedReportDate: report.reportDateCandidates.length === 1 ? report.reportDateCandidates[0] : null,
...
export function extractReport(source: ReportSource, report: SyntheticReport) {
  ...
  if (!validation.supported)
    return { status: 'FAILED_REVIEW' as const, validation, facts: null, exceptions: validation.reasons.map((reason) => exception(reason, reason.replaceAll('_', ' '))) };
  ...
  const tradelines = report.tradelines.map((line) => {
    ...
    if ((line.confidence ?? 'high') === 'low')
      exceptions.push(exception(`AMBIGUOUS_${line.candidateId}`, 'Material tradeline field is ambiguous', [directEvidence]));
    return { ...line, confidence: line.confidence ?? 'high', evidence: [directEvidence], rawValuePreserved: true };
  });
  return {
    status: 'SUCCEEDED' as const,
    validation,
    facts: {
      metadata: { detectedReportDate: validation.detectedReportDate, acceptedReportDate: source.acceptedReportDate },
      scores: report.scores.map((score) => ({ ...score, evidence: [evidence(score.bureau, score.page, 'credit-score')] })),
      tradelines,
      inquiries: report.inquiries.map((item) => ({ ...item, evidence: [evidence(item.bureau, item.page, 'inquiry')] })),
      negativeItems: report.negativeItems.map((item) => ({ ...item, evidence: [evidence(item.bureau, item.page, 'negative-item')] })),
    },
    exceptions,
    provenance: { reportDocumentId: source.reportDocumentId, clientId: source.clientId, sourceVersion: source.sha256, processKey: REPORT_PROCESS_VERSIONS.extract.key, processVersion: 1, outputSchemaVersion: 1 },
  };
}

export function staleExtraction<T extends { provenance: { sourceVersion: string } }>(artifact: T, currentSha256: string) {
