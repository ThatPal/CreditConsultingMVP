...
const create = (runtime: AIRuntime) => runtime.createJob({ clientId: 'client-a', correlationId: 'corr', relatedEntityId: 'report', sourceIdentity: 'sha256:a', process, input: {}, sourceVersions: { report: 'sha256:a' } });

describe('governed AI runtime', () => {
  test('validates, succeeds, stores provenance, and is duplicate safe', async () => {
    const runtime = new AIRuntime(new DeterministicAIProvider(() => response), { 'fixture.process@1': (value) => (value as { valid?: boolean }).valid === true });
  ...
  test('transient failure is retryable and recovery requeues it', async () => {
    let attempt = 0;
    const runtime = new AIRuntime(new DeterministicAIProvider(() => { if (!attempt++) throw new AIProviderError('down', true, 'PROVIDER_DOWN'); return response; }), { 'fixture.process@1': () => true });
    const job = create(runtime);
  ...
  test('invalid output, stale context, disabled provider and cross-client reads fail closed', async () => {
    const runtime = new AIRuntime(new DeterministicAIProvider(() => response), { 'fixture.process@1': () => false });
    const job = create(runtime);
    ...
    expect(() => create(new AIRuntime(null, {}))).toThrowError(AIProviderError);
  });
});
