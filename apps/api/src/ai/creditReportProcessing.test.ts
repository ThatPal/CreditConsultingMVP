...
import {
  encryptedReport,
  malformedReport,
  supportedThreeBureauReport,
  unreadableReport,
  unsupportedReport,
} from './fixtures/syntheticReports.js';

const source = {
  reportDocumentId: 'report-a',
  clientId: 'client-a',
  sha256: 'sha-a',
...
describe('credit report validation and factual extraction', () => {
  test('adapts only the explicitly supported uploaded synthetic source and fails others closed', () => {
    const bytes = Buffer.from(
      `%PDF-1.7\nCREDIT_SYNTHETIC_3_BUREAU_V1\n${JSON.stringify(supportedThreeBureauReport)}\n%%EOF`,
    );
    expect(parseSupportedReportBytes(bytes).format).toBe('SYNTHETIC_3_BUREAU_V1');
    expect(parseSupportedReportBytes(Buffer.from('%PDF-1.7 arbitrary report')).format).toBe(
      'UNSUPPORTED_UPLOADED_SOURCE',
    );
    expect(
      parseSupportedReportBytes(Buffer.from('%PDF-1.7\nCREDIT_SYNTHETIC_3_BUREAU_V1\n{bad')).format,
    ).toBe('MALFORMED_UPLOADED_SOURCE');
  });
  test('extracts three bureaus with immutable source, bureau identity and direct evidence', () => {
    ...
    expect(output.facts?.scores.map(({ bureau }) => bureau)).toEqual([
      'EXPERIAN',
      'EQUIFAX',
      'TRANSUNION',
    ]);
  ...
  test.each([
    [malformedReport, 'INCOMPLETE_STRUCTURE'],
    [unreadableReport, 'UNREADABLE'],
    [encryptedReport, 'ENCRYPTED'],
    [unsupportedReport, 'UNSUPPORTED_FORMAT'],
  ] as const)('routes invalid fixture without changing accepted source truth', (report, reason) => {
