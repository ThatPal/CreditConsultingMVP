...
describe('authenticated route readiness', () => {
  test('preserves role-specific location copy', () => {
    expect(routeReadyCopy('/app/cards/wishlist')).toMatchObject({
      label: 'client portal page',
      home: '/app',
    });
    expect(routeReadyCopy('/crm/support')).toMatchObject({ label: 'CRM workspace', home: '/crm' });
    expect(routeReadyCopy('/admin/payments')).toMatchObject({
  ...
  test('renders purposeful recovery instead of a blank document after a route failure', () => {
    render(
      <MemoryRouter initialEntries={['/app/cards/wishlist']}>
        <RouteReadyBoundary>
          <BrokenRoute />
        </RouteReadyBoundary>
      </MemoryRouter>,
    ...
    expect(screen.getByText(/Reloading clears unsaved changes/i)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /retry page/i })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /return to portal home/i })).toHaveAttribute(
      'href',
      '/app',
    );
  });
  ...
  test('keeps the D0 recovery contract when a lazy route family fails to load', async () => {
    ...
    expect(screen.getByRole('button', { name: /retry page/i })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /return to Admin home/i })).toHaveAttribute(
      'href',
      '/admin',
    );
  });
