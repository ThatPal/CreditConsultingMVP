...
export function SecurityPage() {
  ...
  const [confirmation, setConfirmation] = useState<
    { kind: 'one'; session: Session } | { kind: 'others' } | { kind: 'current' } | null
  >(null);
  ...
  return (
    <Stack spacing={3}>
      <PageHeader
        ...
        description="Review signed-in devices, recent activity, and account access."
      />
      {notice && (
      ...
      {revoke.isError && (
        <Alert severity="error">
          {revoke.error instanceof Error
            ...
            : 'The session change failed. Please retry.'}
        </Alert>
      )}
      <SectionCard>
        <Stack spacing={2}>
          ...
          <Box>
            ...
            <Typography color="text.secondary" sx={{ mt: 0.5 }}>
              End access for a device you no longer recognize or use.
            </Typography>
          </Box>
          {sessions.isLoading && (
                ...
                <Stack direction="row" spacing={1.5} sx={{ minWidth: 0 }}>
                  ...
                  <Box sx={{ minWidth: 0 }}>
                    ...
                    >
                      ...
                      {session.isCurrent && (
                        <Chip size="small" color="primary" label="Current session" />
                      )}
                    </Stack>
                    <Typography variant="body2" color="text.secondary">
                      Signed in {formatTime(session.createdAt)} · Last active{' '}
                    ...
                    >
                      {session.isCurrent
                        ? 'This is the session you are using now.'
                        : 'Revoke this session if you no longer recognize or use this device.'}
                    </Typography>
                  </Box>
                </Stack>
                <Button
                  color="error"
                  variant="outlined"
                  startIcon={session.isCurrent ? <LogoutRounded /> : undefined}
                  onClick={() =>
                    setConfirmation(
                      session.isCurrent ? { kind: 'current' } : { kind: 'one', session },
                    )
                  }
                >
          ...
          <Divider />
          <Button href="/forgot-password" variant="outlined" sx={{ alignSelf: 'flex-start' }}>
            Reset password
          </Button>
        </Stack>
      </SectionCard>
      <Dialog
        ...
        onClose={() => !revoke.isPending && setConfirmation(null)}
        aria-labelledby="session-confirmation-title"
      >
        <DialogTitle id="session-confirmation-title">
          ...
          <Typography>
            {confirmation?.kind === 'current'
              ...
              : confirmation?.kind === 'others'
                ? 'Every other signed-in device will need to authenticate again.'
                : 'That device will lose access on its next protected request.'}
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setConfirmation(null)} disabled={revoke.isPending}>
            Cancel
          </Button>
          <Button
            color="error"
            variant="contained"
            onClick={() => confirmation && revoke.mutate(confirmation)}
