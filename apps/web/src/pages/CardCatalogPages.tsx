...
export function InsightOperationsPage({ canApprove = false }: { canApprove?: boolean }) {
  const client = useQueryClient();
  const query = useQuery({ queryKey: ['card-insights'], queryFn: () => apiRequest<{ insights: Array<{ id: string; status: string; version: number; clientSafeSummary: string; internalRationale: string | null; confidence: string | null; modelProvenance: unknown; evidence: unknown; product: { displayName: string; issuer: { name: string } }; offerVersion: { version: number; status: string }; processDefinition: { processKey: string; processVersion: number; modelProfile: string } | null }> }>('/api/v1/catalog/insights') });
  const approve = useMutation({ mutationFn: (id: string) => apiRequest(`/api/v1/catalog/insights/${id}/approve`, { method: 'POST', body: JSON.stringify({ note: 'Human review completed', idempotencyKey: crypto.randomUUID() }) }), onSuccess: () => client.invalidateQueries({ queryKey: ['card-insights'] }) });
  if (query.isLoading) return <LoadingSkeleton />;
  if (query.isError) return <Alert severity="error">CardInsight review is unavailable or unauthorized.</Alert>;
  return <Stack spacing={3}><PageHeader eyebrow="Card intelligence" title="Insight review" description="AI may prepare structured intelligence, but only authorized professional human review can make it canonical." />
    {!query.data?.insights.length ? <Alert severity="info">No insight versions are awaiting review.</Alert> : query.data.insights.map((insight) => <SectionCard key={insight.id}><Stack spacing={1.5}><Stack direction="row" spacing={1}><Chip label={insight.status} color={insight.status === 'STALE' ? 'warning' : insight.status === 'APPROVED' ? 'success' : 'default'} /><Chip label={`Offer v${insight.offerVersion.version}`} /></Stack><Typography variant="h4">{insight.product.displayName}</Typography><Typography>{insight.clientSafeSummary}</Typography>{insight.internalRationale && <Typography color="text.secondary">Internal review rationale: {insight.internalRationale}</Typography>}<Typography variant="caption">{insight.processDefinition ? `${insight.processDefinition.processKey} v${insight.processDefinition.processVersion} · ${insight.processDefinition.modelProfile}` : 'Manual preparation'}</Typography>{canApprove && ['PREPARED', 'IN_REVIEW'].includes(insight.status) && <Button variant="contained" onClick={() => approve.mutate(insight.id)} disabled={approve.isPending}>Approve as canonical</Button>}{!canApprove && ['PREPARED', 'IN_REVIEW'].includes(insight.status) && <Alert severity="info">Admin can inspect operations, but professional approval requires the consultant catalog-intelligence authority.</Alert>}</Stack></SectionCard>)}
  </Stack>;
}
