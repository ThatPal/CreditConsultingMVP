  ...
  const confirmGoal = useMutation({
    mutationFn: async () => {
      const goalPayload = {
        ...
        cardTypePreference: primaryGoal?.cardTypePreference ?? 'NO_PREFERENCE',
        offerPreferences: goalZeroApr
          ? [...new Set([...(primaryGoal?.offerPreferences ?? []), 'ZERO_APR' as const])]
          : (primaryGoal?.offerPreferences ?? []).filter((item) => item !== 'ZERO_APR'),
        feePreference: goalAllowAnnualFee
          ? primaryGoal?.feePreference === 'NO_ANNUAL_FEE_ONLY'
            ? 'PREFER_NO_FEE_OPEN'
            : (primaryGoal?.feePreference ?? 'PREFER_NO_FEE_OPEN')
          : 'NO_ANNUAL_FEE_ONLY',
        preferenceNote: primaryGoal?.preferenceNote ?? null,
        priority: 'PRIMARY',
      };
      if (primaryGoal) {
        await apiRequest(`/api/v1/client/goals/${primaryGoal.id}`, {
          ...
          body: JSON.stringify({ ...goalPayload, version: primaryGoal.version }),
        });
      } else {
        await apiRequest('/api/v1/client/goals', {
          method: 'POST',
          headers: { 'Idempotency-Key': crypto.randomUUID() },
          body: JSON.stringify({ ...goalPayload, goalType: 'TOTAL_AVAILABLE_CREDIT' }),
        });
      }
