...
| D1316-34 | P1 | Exact-head Linux CI exposed retained queue work and unrelated pending events crossing integration-test ownership boundaries. | Make queue identity and claimed event IDs injectable for deterministic test isolation, and bound startup to one owned event; preserve unrestricted production claims, normal 25-event polling and restart identity. |
