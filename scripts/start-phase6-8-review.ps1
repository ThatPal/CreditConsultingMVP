...
$runtimeDirectory = Join-Path $workspace '.tmp\phase6-8-review'
New-Item -ItemType Directory -Force -Path $runtimeDirectory | Out-Null

$env:DATABASE_URL = 'postgresql://credit:credit_dev@localhost:5433/credit_strategy_phase6_8_block?schema=public'
...
if ($env:DATABASE_URL -match 'behfar' -or $env:DATABASE_URL -notmatch 'credit_strategy_phase6_8_block') {
  throw 'Refusing to start: the Phase 6-8 review environment must use only its Credit database.'
}
...
try {
  ...
  Write-Output 'Phase 6-8 Credit review environment started: Web http://localhost:5184, API http://localhost:3007.'
